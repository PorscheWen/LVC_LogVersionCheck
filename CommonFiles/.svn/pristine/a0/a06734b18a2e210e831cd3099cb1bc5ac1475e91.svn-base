...
def Stage_004_SetProcess():
    def SubStage_SetProject():
        # Porject Setting: 
        ProPara_IM.CAE_MaxPressure(256, 256)
        Aliases.MdxPro.wndAfx.btn_Next.ClickButton()
    def SubStage_SetFP():
        # Flow/Pack Setting:
        ProPara_IM.FlowPackTime(0.25, 5)
    ...
    def SubStage_SetCool():
        ...
        ProPara_IM.CoolPara(MBFlag=1, tCool=20, tMBOpen=5, AirTemp=25, EjTemp=108.85)
...
def Stage_005_SetCMX():
    ...
    cmxFP.Set_TimeStep(Filling=[0.25, 0], Packing=[5, 0])

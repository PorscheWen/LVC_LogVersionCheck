...
def AutoReadMoldexVersion():
    ...
    pyHKEY = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r'SOFTWARE\Wow6432Node\CoreTechSystem\MDX_ParallelComputing')
    valueInfo = winreg.QueryInfoKey(pyHKEY) # Read the value infomation(name & data) under the subKey
    
    versionPathList = []
    for i in range(0, valueInfo[1], 1):
        value = winreg.EnumValue(pyHKEY, i)
        if "_INSTALLDIR" in value[0]:
            versionPathList.append(value[1])
    
    winreg.CloseKey(pyHKEY)

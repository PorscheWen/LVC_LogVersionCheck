...
class InletSetting:
    ...
    def __init__(self, InletNum):
        Aliases.MdxPro.wndAfx.SysTabControl32.SetFP.SysListView32.ClickItem(
            "%s Material" % InletNum, 0)
        self.WndObj = Aliases.MdxPro.wndAfx.SysTabControl32.SetFP.Temp
        if InletNum == "Skin":
            self.lstMeltDown = [(3, 4, 7, 8)]
        elif InletNum == "Core":
            self.lstMeltDown = [(3, 4, 12, 9), (7, 16), (7, 10, 16)]
        self.Col = 1
#==============================================================
    ...
    def Set_MaxFlowRate(self, maxFlowRate):
        self.iDown = self.lstMeltDown[0][1]
        self.set_Text(maxFlowRate)
    ...
    def Set_VPSwitchValue(self, valVP):
        self.iDown = self.lstMeltDown[1][0]
        self.Col = 2
        self.set_Text(valVP)

    def Set_CoreEnterTime(self, tCoreIn):
        self.iDown = self.lstMeltDown[1][1]
        self.Col = 2
        self.set_Text(tCoreIn)        
#==============================================================

    def Set_VPSwitch(self, Type):
        self.iDown = self.lstMeltDown[2][0]
        self.click_Item(Type)

    def Set_RefPackPr(self, refPr):
        self.iDown = self.lstMeltDown[2][1]
        self.click_Item(refPr)

    def Set_CoreEnter(self, Type):
        self.iDown = self.lstMeltDown[2][2]
        self.click_Item(Type)
#==============================================================
    ...
    def move_Row(self):
        ...
        if self.Col != 1:
            self.WndObj.Click(361, 12)    
        else:
            self.WndObj.Click(255, 12)
        if self.iDown != 0:
...
class ProfileSetting():
    ...
    def __init__(self, pfType):
        ...
        if pfType == "Skin":
            self.dicProfilePos = {
                "FlowRate": [0, (255, 117)],
                "InjactionPressure": [0, (255, 137)]}
        elif pfType == "Core":
            self.dicProfilePos = {
                "FlowRate": [0, (255, 117)],
                "InjactionPressure": [0, (255, 137)],
                "PackPressure": [195, (255, 33)]}

...
def Stage_004_SetProcess():
    def SubStage_SetProject():
        # Porject Setting: 
        ProPara_IM.CAE_MaxPressure(155, 90)
        Aliases.MdxPro.wndAfx.btn_Next.ClickButton()
    def SubStage_SetFP():
        # Flow/Pack Setting:
        ProPara_IM.FlowPackTime(0.1, 0.5)
        ...
        tupTime = (0.1, 0)
        tupValue = (120, 120)
        ProPara_IM.CAE_Profile("InjactionPressure", tupTime, tupValue, TypeTag="Injection Pressure (MPa) vs Time (sec)*", SectionNo=1)
        tupTime = (0.5, 0.25, 0)
        tupValue = (60, 80, 80)        
        ProPara_IM.CAE_Profile("PackPressure", tupTime, tupValue, "RefMachine", TypeTag="Packing Pressure (MPa) vs. Time (sec)*", SectionNo=2)
        Aliases.MdxPro.wndAfx.btn_Next.ClickButton()
    def SubStage_SetCool():
        ...
        ProPara_IM.CoolPara(MBFlag=1, tCool=25.5, tMBOpen=5, AirTemp=25, EjTemp=99.85)
        CoolAdv = ProPara_IM.CoolPara_Adv()
        CoolAdv.MoldMetalMTR(1, "P6")
        CoolAdv.CC(1, lstT=[20])
        Aliases.MdxPro.wndAfx.btn_Next.ClickButton()
...
def Stage_005_SetCMX():
    ...
    cmxFP.Set_TimeStep(Filling=[0.1, 0], Packing=[0.5, 0])
